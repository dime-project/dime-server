...
/**
 * Tests {@link SnapshotBasedStrategy}.
 */
public class SnapshotBasedStrategyTest extends SemanticTest {
	...
	public void setUp() throws Exception {
		...
		this.strategy = new SnapshotBasedStrategy(previousContext, liveContext);
	}
	...
	public void testUpdateSeveralResources() throws LiveContextException {
		...
		weatherConditionsURI = (URI) this.liveContext.findStatements(aspectURI, DCON.currentWeather, Variable.ANY).next().getObject();
		assertTrue(this.liveContext.findStatements(aspectURI, DCON.currentTemperature, Variable.ANY).hasNext());
		temperatureURI = (URI) this.liveContext.findStatements(aspectURI, DCON.currentTemperature, Variable.ANY).next().getObject();
		...
		weatherConditionsURI = (URI) this.previousContext.findStatements(aspectURI, DCON.currentWeather, Variable.ANY).next().getObject();
		assertTrue(this.previousContext.findStatements(aspectURI, DCON.currentTemperature, Variable.ANY).hasNext());
		temperatureURI = (URI) this.previousContext.findStatements(aspectURI, DCON.currentTemperature, Variable.ANY).next().getObject();
		...
		toRemove.clear();
		toAdd.add(new StatementImpl(null, weather, DCON.cloudcover, new DatatypeLiteralImpl("5", XSD._int)));
		toRemove.add(new StatementImpl(null, weather, DCON.cloudcover, new DatatypeLiteralImpl("10", XSD._int)));
		strategy.update(toAdd, toRemove);
		...
		assertTrue(this.previousContext.findStatements(temperatureURI, DCON.temperature, new DatatypeLiteralImpl("36.0", XSD._double)).hasNext());
		assertTrue(this.previousContext.findStatements(weatherConditionsURI, RDF.type, DPO.WeatherConditions).hasNext());
		assertTrue(this.previousContext.findStatements(weatherConditionsURI, DCON.cloudcover, new DatatypeLiteralImpl("10", XSD._int)).hasNext());
